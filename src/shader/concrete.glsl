...
float sdfTopoWall(vec3 p) {
    ...
    float t = clamp(terrain(p.xy), -.5, 0.5);
    t = terrain(p.xy);
    t = fbm(p.xy, 1.);

    float planeDist = p.z + 5.;

    // if (t >= .5) {
    //     // vec4 n = noised(p);
    //     // t = terrain(p.xy * 2.);
    //     // return planeDist + t * n.w * 21.;
    //     return MAX_DIST;
    //     // return MAX_DIST;
    // }

    return planeDist + t;
    // return p.z + 5. + terrain(p.xy);
...
float sdfstuff(vec3 p, float r) {
    return distance(p, vec3(0.))-r;
}

float sdf(vec3 p) {
    float dist = sdfTopoWall(p);
    // float dist = sdfstuff(p, 0.8);
    return dist;
}
